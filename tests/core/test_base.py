...
def test_sort_tasks_single_task_no_dependencies():
...
def test_sort_tasks_multiple_tasks_no_dependencies():
...
def test_sort_tasks_linear_dependencies():
...
def test_sort_tasks_branching_dependencies():
...
def test_sort_tasks_converging_dependencies():
...
def test_sort_tasks_complex_dag():
...
def test_sort_tasks_cycle_detection():
    ...
    with pytest.raises(
        ValueError,
        match="Graph has no tasks without dependencies, indicating a cycle or an invalid DAG.",
    ):
        _sort_tasks(tasks)


def test_sort_tasks_no_initial_dependencies():
    """Test that _sort_tasks raises ValueError if no task has initial dependencies (not a DAG)."""
    # This scenario implies a graph where all nodes have incoming edges, which is a cycle or disconnected components
...
def test_sort_tasks_disconnected_components():
    ...
    sorted_tasks = _sort_tasks(tasks)
    # A and B must come before C and D respectively. The relative order of A/B and C/D can vary.
