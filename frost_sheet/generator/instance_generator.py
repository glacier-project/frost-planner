...
from frost_sheet.core.base import Job, Task, Machine
...
class InstanceGenerator:
    ...
    def create_instance(self, configuration: InstanceConfiguration = InstanceConfiguration()) -> tuple[list[Job], list[Machine]]:
        # generate machines
        machines = []
        capabilities = []
        for i in range(configuration.num_machine_capabilities):
            ...
            num_machines = random.randint(configuration.min_machine_per_capability, configuration.max_machine_per_capability)
        ...
        for i in range(configuration.num_jobs):
            num_tasks = random.randint(configuration.min_tasks_per_job, configuration.max_tasks_per_job)
            num_tasks_without_dependencies = random.randint(configuration.min_task_without_dependencies, configuration.max_task_without_dependencies)
            tasks = []

            for j in range(num_tasks):
                processing_time = random.randint(configuration.min_processing_time, configuration.max_processing_time)
                ...
                ) if j > num_tasks_without_dependencies else []
                ...
                task = Task(
                    ...
                    requires=requires,
                    priority=random.randint(configuration.min_task_priority, configuration.max_task_priority),
                )
            ...
            jobs.append(Job(
                ...
                name=f"job_{i}",
                tasks=tasks,
                priority=random.randint(configuration.min_job_priority, configuration.max_job_priority)
            ))
