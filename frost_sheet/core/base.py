...
def _generate_unique_task_ids(jobs: list["Job"]) -> list["Job"]:
    """
    Make task IDs unique across all jobs by generating new UUIDs.
    This function takes a list of Job objects and reassigns all task IDs within
    those jobs to ensure uniqueness. It also updates any task dependencies to
    reference the new IDs, maintaining the dependency relationships.

    Args:
        jobs (list[Job]): A list of Job objects containing tasks with potentially
                         duplicate IDs.
    Returns:
        list[Job]: The same list of Job objects with all task IDs made unique
                  and dependencies updated accordingly.
    """
...
def _sort_tasks(tasks: list["Task"]) -> list["Task"]:
    ...
    This function assumes that the input list of tasks is a directed acyclic graph (DAG).
    """
...
class Task(BaseModel):
    ...
    Attributes:
        ...
        processing_time (int): The time required to process the task.
        dependencies (list[str]): The identifiers of the tasks that must be completed before this task can start.
        requires (list[str]): The capabilities required to complete the task.
        machines (list[str]): The identifiers of the machines that can process this task.
...
class Job(BaseModel):
    ...
    Attributes:
        job_id (str): A global unique identifier for the job.
    ...
    def start_time(self) -> int:
        """Returns the earliest start time of all tasks in the job. If no tasks are present or all tasks have no start time, returns 0.

        Returns:
            int: The earliest start time of all tasks in the job.
        """
        start_time = None
    ...
    def end_time(self) -> int:
        """Returns the latest end time of all tasks in the job. If no tasks are present or all tasks have no end time, returns 0.

        Returns:
            int: The latest end time of all tasks in the job.
        """
        end_time = 0
...
class Machine(BaseModel):
    ...
    Attributes:
        machine_id (str): A global unique identifier for the machine.
